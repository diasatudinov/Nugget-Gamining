...
struct NGShopView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                }.frame(height: NGDeviceManager.shared.deviceType == .pad ? 600:300)
            }
    ...
    @ViewBuilder func shopSpecialItem(special: NGItem) -> some View {
        ZStack {
            Image("\(special.icon)")
                .resizable()
                .scaledToFit()
                ...
                HStack {
                    ...
                    ZStack {
                        Image(.itemLevelBgNG)
                            ...
                            .scaledToFit()
                        Text("\(special.level)")
                            .font(.system(size: NGDeviceManager.shared.deviceType == .pad ? 20:12))
                            .foregroundStyle(.white)
                    }.frame(height: NGDeviceManager.shared.deviceType == .pad ? 60:33)
                }
                Spacer()
            }
            VStack {
                ...
                Button {
                    ...
                    Image(viewModel.isMoneyEnough(item: special, user: user, category: .special) ? .tenBtnNG: .tenOffBtnNG)
                        .resizable()
                        .scaledToFit()
                        .frame(height: NGDeviceManager.shared.deviceType == .pad ? 40:27)
                    
                }
                .offset(y: 20)
            }
        }
        .frame(width: NGDeviceManager.shared.deviceType == .pad ? 180:100 ,height: NGDeviceManager.shared.deviceType == .pad ? 200:120)
    }
    ...
    @ViewBuilder func shopBgItem(bg: NGItem) -> some View {
        ...
        .frame(height: NGDeviceManager.shared.deviceType == .pad ? 170:95)
    }
...
#Preview {
    NGShopView(viewModel: NGShopViewModel())
}
